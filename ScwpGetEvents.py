...
def getCredsFromSplunkStorage():
    ...
    if len(current_credentials) == 0:
        ...
        for current_credential in current_credentials:
            scwpCreds = {'clientId': current_credential.content.get('username'),
...
try:
    ...
    while True:
        ...
        if retry in range(EVENT_GET_RETRY_COUNT):
            scwpEventsResponse = requests.post(getScwpEventsUrl, data=getScwpEventsRequestJson, headers=authHeaders)

            if scwpEventsResponse.status_code == 504:
                print("will retry after some time ",retry)
                time.sleep(retry * 60)
                retry = retry + 1
                continue
            retry = 0
            ...
            print("SCWP event service is busy. Please try after some time.")
            break
except:
    raise
finally:
